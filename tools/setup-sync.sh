#!/usr/bin/env bash
# Configure wireless (Tailscale) save sync between this Mac and the cc-ios app on a
# connected iPhone — no hand-editing JSON, no hard-coded IPs.
#
# It:
...
#
# Pair it with the server (see tools/save-server.py): run `tools/setup-sync.sh --serve`
# to also start the server now, or `--install-service` to run it persistently via launchd.
#
# Usage:
...
#   tools/setup-sync.sh --url http://host:8765  # set the full URL explicitly
...
if [[ "$do_service" -eq 1 ]]; then
  CCIOS_SYNC_TOKEN="$token" "$repo_root/tools/save-server.sh" install --port "$port"
elif [[ "$do_serve" -eq 1 ]]; then
  echo "Starting save-server (Ctrl-C to stop)…"
  exec env ${token:+CCIOS_SYNC_TOKEN="$token"} python3 "$repo_root/tools/save-server.py" --port "$port"
else
  echo
  echo "Next: start the save-server so the Mac is reachable:"
  echo "  tools/save-server.py --port $port            # foreground"
