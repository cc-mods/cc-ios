#!/usr/bin/env bash
# Two-way CrossCode save sync between this Mac (Steam copy) and a USB-connected iPhone
# running cc-ios, using `xcrun devicectl device copy`. Last-writer-wins by modification time.
#
# The iOS app keeps its save at Documents/cc.save inside its app container; that file is
# byte-identical to the desktop save the Steam game reads/writes. Steam Cloud then
...
# Warm up the device tunnel + developer disk image services. The FIRST devicectl file
# operation after connecting can stall while these initialise; this absorbs that delay so
# the actual copies below run fast.
